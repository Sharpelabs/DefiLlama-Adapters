...
const CouchDBClient = require("./utils/couchDb");
...
async function addTvl(id, projectPath, dbPath) {
  const passedFile = `${process.cwd()}/${projectPath}`;
  ...
  await couchClient.insert(`defi-${dbPath}/${id}`, {
...
(async () => {
  for (let item of ADAPTORS) {
    console.log(ADAPTORS.indexOf(item), item.name, item.id, item.symbol);
    try {
      await addTvl(item.id, `projects/${item.module}`, "tvl");
    } catch (e) {
      console.log(`Error ${e.message}`);
    }
  }
  // const directoryPath = '/home/ubuntu/DefiLlama-Adapters/projects/treasury'
  // const pathFiles = fs.readdirSync(directoryPath);
  // const files = pathFiles.filter(file => fs.statSync(path.join(directoryPath, file)).isFile());

  for (let item of ADAPTORS) {
    if ("treasury" in item && item.treasury && ADAPTORS.indexOf(item) >= 4200) {
      console.log(ADAPTORS.indexOf(item), item.name, item.treasury);
      try {
        await addTvl(item.id, `projects/treasury/${item.treasury}`, "treasury");
      } catch (e) {
...
const { encodeIdlAccount } = require("@project-serum/anchor/dist/cjs/idl");
...
async function initCache() {
  ...
  if (
    ...
    Date.now() / 1000 - currentCache.startTime > ONE_WEEK
  ) {
    currentCache = {
...
async function preExit() {
  try {
    ...
    if (process.env.NO_EXIT_ON_LONG_RUN_RPC) sdk.error(e);
  }
}
